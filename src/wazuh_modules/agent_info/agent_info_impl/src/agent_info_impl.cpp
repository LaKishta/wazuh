...
void AgentInfoImpl::updateDbMetadata()
{
    ...
    DBSyncTxn txn{dbSync->handle(), nlohmann::json{"db_metadata"}, 0, QUEUE_SIZE, callback};
...
void AgentInfoImpl::setSyncFlag(const std::string& table, bool value)
{
    ...
    try
    {
        ...
        {
            m_logFunction(LOG_DEBUG, "First run detected for " + table + ", skipping synchronization.");
            m_isFirstRun = false;
        ...
        if (m_isFirstGroupsRun && value && table == AGENT_GROUPS_TABLE)
        {
            m_logFunction(LOG_DEBUG, "First groups run detected for " + table + ", skipping synchronization.");
